 ...
 */
public final class Astar extends AbstractStateSpacePlanner {
     ...
     */
    public Astar(final boolean statisticState, final int traceLevel) {
     ...
     * @param traceLevel     the trace level of the planner.
     */
    public Astar(final int timeout, final Heuristic.Type heuristicType, final double weight,
    ...
    public SequentialPlan search(final CodedProblem problem) {
        ...
        if (solutionNode != null) {
        	Logs.log(LogType.INFO, "* succeed A*\n");
            return (SequentialPlan) astar.extractPlan(solutionNode, problem);
        } else {
        	Logs.log(LogType.ERROR, "* A* failed\n");
            return null;
        }
    }
}